#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# build-dmg.sh — Full MaskBase build: Python backend → Tauri .dmg
#
# Usage:
#   ./build-dmg.sh          # build only (fast, no signing)
#   ./build-dmg.sh --sign   # build + codesign + notarize
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
# ── 4. Build the Python backend binary ───────────────────────────────────────
echo ""
...
# ── 5. Copy binary to src-tauri/binaries/ with Tauri naming convention ───────
echo ""
...
# ── 7. Configure Apple Developer ID signing (if --sign) ──────────────────────
if $SIGN_APP; then
  echo ""
...
echo ""
echo "── Step 7: Building Tauri app (Rust compile + bundle) ───────────────────"
...
if [[ -n "$DMG_PATH" ]]; then
  ...
  echo ""
  echo "    Share this file with your users — no Python or dependencies needed."
else
